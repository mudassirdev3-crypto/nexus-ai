...
import { LayoutDashboard, Users, Send, Settings, X, Plus, Bot, ChevronRight, Trash2, Activity, Globe } from "lucide-react";
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-white flex font-sans selection:bg-blue-500/30">
      <Toaster position="top-right" richColors theme="dark" />
      
      {/* --- SIDEBAR --- */}
      <aside className="w-72 border-r border-white/5 p-8 flex flex-col gap-10 bg-[#050505] hidden lg:flex">
      ...
      <main className="flex-1 p-8 lg:p-12 overflow-y-auto">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
          ...
          <button 
            ...
            className="group bg-white text-black px-8 py-4 rounded-2xl text-sm font-black hover:bg-blue-600 hover:text-white transition-all flex items-center gap-2 active:scale-95 shadow-xl hover:shadow-blue-600/20"
          >
        ...
        {/* --- AGENTS SECTION --- */}
        <div className="bg-[#080808] rounded-[2.5rem] border border-white/5 p-8 lg:p-10 shadow-2xl">
          <div className="flex justify-between items-center mb-10">
            <h3 className="text-2xl font-black tracking-tight">Active Neural Units</h3>
            <div className="flex gap-2 text-gray-500 text-xs font-bold uppercase tracking-widest">
            ...
            {agents.length === 0 ? (
                ...
                agents.map((agent) => (
                    <motion.div 
                      ...
                      key={agent.id} 
                      className="group flex items-center justify-between p-6 bg-white/[0.01] rounded-3xl border border-white/5 hover:bg-white/[0.03] hover:border-blue-500/20 transition-all cursor-default shadow-sm"
                    >
                      <div className="flex items-center gap-6">
                        <div className="h-16 w-16 bg-gradient-to-br from-blue-600/10 to-transparent rounded-[1.5rem] flex items-center justify-center text-blue-500 border border-blue-500/10 group-hover:scale-105 transition-transform">
                          <Bot size={30} />
                        </div>
                        <div>
                          <h4 className="font-black text-xl tracking-tight">{agent.name}</h4>
                          <div className="flex items-center gap-3 mt-1">
                            <span className="text-[10px] text-gray-500 font-black uppercase tracking-widest">{agent.role}</span>
                            <span className="h-1 w-1 bg-gray-700 rounded-full"></span>
                            <span className="text-[10px] text-blue-500 font-black uppercase tracking-widest tracking-tighter">v1.0.4</span>
                          </div>
                        </div>
                      </div>
                      
                      <div className="flex items-center gap-4">
                          {/* DELETE BUTTON - Always Visible with better UI */}
                          <button 
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            <motion.div 
              ...
              className="relative bg-[#0a0a0a] border border-white/10 p-10 rounded-[3rem] w-full max-w-lg shadow-2xl"
            >
              <div className="flex justify-between items-center mb-10">
                <div>
                  <h3 className="text-3xl font-black tracking-tight">Deploy Agent</h3>
                  <p className="text-gray-500 text-sm mt-2 font-medium">Specify parameters for your neural unit.</p>
                </div>
                <button onClick={() => setIsModalOpen(false)} className="h-12 w-12 flex items-center justify-center bg-white/5 rounded-full text-gray-500 hover:text-white hover:bg-red-500/20 transition-all">
                  <X size={20} />
                </button>
              </div>
              ...
              <form onSubmit={handleAddAgent} className="space-y-8">
                <div className="space-y-3">
                  ...
                  <input required type="text" placeholder="Nexus-01" className="w-full bg-white/[0.03] border border-white/5 p-5 rounded-2xl focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none transition-all font-bold"
                ...
                <div className="space-y-3">
                  ...
                  <input required type="text" placeholder="Sales Executive" className="w-full bg-white/[0.03] border border-white/5 p-5 rounded-2xl focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none transition-all font-bold"
...
function StatCard({ label, value, trend, color = "text-white", icon }: any) {
  return (
    <div className="bg-white/[0.02] p-8 rounded-[2.5rem] border border-white/5 relative overflow-hidden group hover:border-white/10 transition-colors">
      <div className="flex justify-between items-start mb-6">
        <div className="p-3 bg-white/5 rounded-xl text-gray-400 group-hover:text-white transition-colors">{icon}</div>
        <span className="text-[10px] font-black bg-white/5 px-3 py-1.5 rounded-full text-gray-400 tracking-widest">{trend}</span>
      </div>
      <p className="text-gray-500 text-[10px] font-black uppercase tracking-[0.2em]">{label}</p>
      <h2 className={`text-5xl font-black tracking-tighter mt-2 ${color}`}>{value}</h2>
